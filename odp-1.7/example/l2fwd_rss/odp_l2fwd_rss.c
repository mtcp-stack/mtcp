 ...
 */
#ifndef HIGH_PERF_MODE
#define SHM_PKT_POOL_BUF_SIZE  1856
#else
#define SHM_PKT_POOL_BUF_SIZE  6144
#endif
/** @def MAX_PKT_BURST
...
typedef struct {
	...
	int queue_index;	/**< queue_index of the port */
...
static void *run_worker_direct_mode(void *arg)
{
	...
	thread_args_t *thr_args = arg;
	stats_t *stats = thr_args->stats;
	int que_recv_packet_count[16] = {0};
	...
	while (!exit_threads) {
		...
		/* rss count */
		que_recv_packet_count[pktin.index] += pkts;
			printf("\nque:%d,recv packets num:%x\n",
			       pktin.index,
			       que_recv_packet_count[pktin.index]);
...
static void parse_args(int argc, char *argv[], appl_args_t *appl_args)
{
	...
	while (1) {
		opt = getopt_long(argc, argv, "+c:+t:+a:i:m:d:s:e:q:h",
				  longopts, &long_index);
		...
		case 'e':
			...
			break;
		case 'q':
			appl_args->queue_index = atoi(optarg);
			break;
		case 'h':
			usage(argv[0]);
			exit(EXIT_SUCCESS);
...
static void
check_all_ports_link_status(uint8_t port_num, uint64_t port_mask)
{
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < num; i++) {
		printf("pktio_info[%d].info.dev_info.if_idx :%d\n",
	...
	for (i = 0; i < if_count; ++i) {
...
#if RSS
		struct odp_eth_ntuple_filter  arg = {
			.flags = ODP_5TUPLE_FLAGS,
		...
		portid = atoi(gbl_args->appl.if_names[i] + strlen("pktio_"));
		for (i = 0; i < FLOW_IN_USE_NUM; i++) {
			arg.dst_ip = IPV4(192, 168, 0, (i + 1));
			arg.queue = i;
			odp_eth_dev_filter_ctrl(portid, ODP_ETH_FILTER_NTUPLE,
					ODP_ETH_FILTER_ADD, (void *)&arg);
		}
	...
	/* cpu = (int)((cpu + gbl_args->appl.queue_index) & 0xf); */
	for (i = 0; i < num_workers; ++i) {
