...
char i40e_driver_name[] = "igb_uio_i40e";
...
int i40e_get_local_mac_addr(struct pci_dev *pdev, const struct pci_device_id *ent, u8 *g_mac_addr)
{
	...
	/**************************************************/
	// This forms part of the i40e_init_module()
	if (i40e_wq == NULL) {
	...
	memcpy(g_mac_addr, hw->mac.addr, ETH_ALEN);
...
EXPORT_SYMBOL(i40e_get_local_mac_addr);
/***************************************************************************/
...
#endif
{
	...
	printk(KERN_ALERT "hw_addr: %02X:%02X:%02X:%02X:%02X:%02X\n",
	       hw->hw_addr[0],
	       hw->hw_addr[1],
	       hw->hw_addr[2],
	       hw->hw_addr[3],
...
EXPORT_SYMBOL(i40e_init_module);
...
EXPORT_SYMBOL(i40e_exit_module);
