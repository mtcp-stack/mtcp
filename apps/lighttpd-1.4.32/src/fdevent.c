...
/*----------------------------------------------------------------------------*/
fdevents *
fdevent_init(server *srv, size_t maxfds, fdevent_handler_t type)
{
	fdevents *ev;
	...
	case FDEVENT_HANDLER_LINUX_SYSEPOLL:
		...
		return ev;
	case FDEVENT_HANDLER_LIBMTCP:
		if (0 != fdevent_libmtcp_epoll_init(ev)) {
			log_error_write(ev->srv, __FILE__, __LINE__, "S",
				"event-handler linux-sysepoll failed, try to set server.event-handler = \"poll\" or \"select\"");
			return NULL;
		}
...
/*----------------------------------------------------------------------------*/
int
fdevent_event_del(fdevents *ev, int *fde_ndx, int fd)
{
...
/*----------------------------------------------------------------------------*/
int
fdevent_event_set(fdevents *ev, int *fde_ndx, int fd, int events)
{
	int fde = fde_ndx ? *fde_ndx : -1;
...
/*----------------------------------------------------------------------------*/
int
fdevent_event_get_revent(fdevents *ev, size_t ndx)
{
...
/*----------------------------------------------------------------------------*/
int
fdevent_event_get_fd(fdevents *ev, size_t ndx)
{
...
/*----------------------------------------------------------------------------*/
fdevent_handler
fdevent_get_handler(fdevents *ev, int fd) 
{
...
/*----------------------------------------------------------------------------*/
void *
fdevent_get_context(fdevents *ev, int fd) 
{
	if (ev->fdarray[fd] == NULL) SEGFAULT();
	if (ev->fdarray[fd]->fd != fd) SEGFAULT();
