...
struct apr_pollset_private_t
{
    int epoll_fd;
#if HAVE_MTCP
	struct mtcp_epoll_event *pollset;
#else
...
#ifdef HAVE_MTCP
    int cpu = sched_getcpu();
	ret = mtcp_epoll_wait(g_mctx[cpu], pollset->p->epoll_fd, pollset->p->pollset, pollset->nalloc,
                     -1);
#else
                                       ...
                                       apr_uint32_t flags)
{
    ...
    pollcb->fd = fd;
#ifdef HAVE_MTCP
	pollcb->pollset.epoll = apr_palloc(p, size * sizeof(struct mtcp_epoll_event));
#else
