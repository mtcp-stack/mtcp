...
static int config_insert(server *srv) {
	...
	config_values_t cv[] = {
		...
		{ "server.max-fds",              NULL, T_CONFIG_INT, T_CONFIG_SCOPE_SERVER },       /* 23 */
#ifdef HAVE_LSTAT
		...
		{ "server.max-connections",      NULL, T_CONFIG_INT, T_CONFIG_SCOPE_SERVER },       /* 43 */
...
#ifdef HAVE_LIBMTCP
		{ "server.listen-backlog",       NULL, T_CONFIG_INT, T_CONFIG_SCOPE_SERVER },       /* 76 */
		{ "server.infinite-keep-alive-requests",      NULL, T_CONFIG_BOOLEAN, T_CONFIG_SCOPE_CONNECTION },       /* 77 */
#endif
...
#ifdef HAVE_LIBMTCP
	cv[76].destination = &(srv->srvconf.listen_backlog);
#endif
...
#ifdef HAVE_LIBMTCP
		s->infinite_keep_alive_requests = 0;
#endif
...
#ifdef HAVE_LIBMTCP
		cv[77].destination = &(s->infinite_keep_alive_requests);
#endif
...
int config_setup_connection(server *srv, connection *con) {
	...
	PATCH(mimetypes);
#ifdef HAVE_LIBMTCP
	PATCH(infinite_keep_alive_requests);
#endif
	PATCH(document_root);
	...
	for (i = 1; i < srv->config_context->used; i++) {
		...
		for (j = 0; j < dc->value->used; j++) {
			...
			if (buffer_is_equal_string(du->key, CONST_STR_LEN("server.document-root"))) {
				...
				PATCH(max_keep_alive_idle);
			} else if (buffer_is_equal_string(du->key, CONST_STR_LEN("server.infinite-keep-alive-requests"))) {
				PATCH(infinite_keep_alive_requests);
...
#endif
#ifdef HAVE_LIBMTCP
		{ FDEVENT_HANDLER_LIBMTCP, "mtcp-epoll" },
#endif
